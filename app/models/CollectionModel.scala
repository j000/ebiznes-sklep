...
object Collections_helper extends EntityTableModule[Long, Collection_helper]("Collections_helper") {
  class Row(tag: Tag) extends BaseEntRow(tag) with AutoNameSnakify {
...
{
  def update(id: Long, data: Collection): Future[Option[DBO]] = db run {
    Collections.Q.filter(_.key === id).map(_.mapping) update (data) map {
      case 0 => None
      case _ => Some(SavedEntity[Long, Collection](id, data))
    }
  }
}
...
object Collection extends ((
  ...
  implicit val _writes: Writes[KeyedEntity[Long, Collection]] = (
    ...
    (JsPath).write[Collection]
  )(unlift(KeyedEntity.unapply[Long, Collection]))
  implicit val _reads: Reads[KeyedEntity[Long, Collection]] = (
    (JsPath \ "id").read[Long] and
    (JsPath).read[Collection]
  )(KeyedEntity.apply[Long, Collection] _)
  implicit val _format: Format[KeyedEntity[Long, Collection]] = Format(_reads, _writes)
}

object Collection_helper extends ((
  Long,
  Long,
) => Collection_helper) {
