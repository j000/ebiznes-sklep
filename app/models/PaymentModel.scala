...
object Payments extends EntityTableModule[Long, Payment]("Payments") {
  class Row(tag: Tag) extends BaseEntRow(tag) with AutoNameSnakify {
    ...
    def mapping = (order, amount, comment) <> ((Payment.apply _).tupled, Payment.unapply)
  }
}
...
{
  def update(id: Long, data: Payment): Future[Option[DBO]] = db run {
    Payments.Q.filter(_.key === id).map(_.mapping) update (data) map {
      case 0 => None
      case _ => Some(SavedEntity[Long, Payment](id, data))
    }
  }
}
...
object Payment {
  ...
  implicit val _format: Format[KeyedEntity[Long, Payment]] = Format(_reads, _writes)
}
