...
{
  def update(id: Long, data: Review): Future[Option[DBO]] = db run {
    Reviews.Q.filter(_.key === id).map(_.mapping) update (data) map {
      case 0 => None
      case _ => Some(SavedEntity[Long, Review](id, data))
    }
  }
}

object Review extends ((
  String,
  Long,
  Long,
) => Review) {
  ...
  implicit val _format: Format[KeyedEntity[Long, Review]] = Format(_reads, _writes)
}
